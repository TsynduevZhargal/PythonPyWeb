...
class Author(models.Model):
    ...
    gender = models.CharField(max_length=1, choices=[('м', 'мужской'), ('ж', 'женский')])   # пол
    self_esteem = models.DecimalField(max_digits=2, decimal_places=1)  # уровень самооценки
    phone_number = models.CharField(max_length=12, validators=[phone_regex])     # номер телефона
    city = models.CharField(max_length=100)                            # город
    bio = models.TextField()
    age = models.IntegerField(null=True, editable=False)               # возраст
    date_birth = models.DateField()                                    # дата рождения
    status_rule = models.BooleanField()                                # статус
    image = models.ImageField(upload_to='foto_profile')                # фото, сохранение на сервере
    create_at = models.DateTimeField(auto_now_add=True)                # дата и время создания
    update_at = models.DateTimeField(auto_now=True)                    # дата и время обновления
